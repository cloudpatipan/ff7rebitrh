...
export default function Blog() {
    ...
    const { blogs, setBlogs } = useContext(FavoriteAndBlogContext);
    ...
    return (
        <>
            {loading ? (
                ...
                <Layout header={'บล็อกของฉัน'}>

                    <div className="w-[95%] mx-auto">


                        <div className={`my-4 flex flex-col md:flex-row ${token ? ' justify-between' : 'justify-end'} items-center gap-4`}>
                            {token && (
                                <div>
                                    <Link href={'/blog/create'}>
                                        <Button name={'เพิ่มบล็อก'} icon={<PiPlusThin size={25} />} />
                                    </Link>
                                </div>
                            )}
                                    ...
                                    <div key={index} className="flex flex-col gap-4">
                                        ...
                                        {blog.user_id === user?.id && (
                                            <div className={`flex items-center justify-center gap-4 p-2 w-full mx-auto border border-[#176db0]`}>
                                                ...
                                                <div>
                                                    <Link href={`/community/blog/${blog.name}`}>
